...
def copy_source_snapshot(manifest_path: Path, manifest: dict[str, str], report_dir: Path) -> Path | None:
    source_path = resolve_manifest_path(manifest.get("source.file_path", ""), manifest_path.parent)
    if source_path is None or not source_path.exists():
        return None
    suffix = source_path.suffix or ".md"
    destination = report_dir / f"source-snapshot{suffix}"
...
def main() -> int:
    ...
    source_snapshot = copy_source_snapshot(manifest_path, manifest, report_dir)
    ...
    request = "\n".join(
        [
            ...
            f"- Source snapshot: {source_snapshot or ''}",
            "",
    ...
    quality_cmd = [
        ...
        "--output-json",
        str(report_dir / "quality-report.json"),
        "--output-pr-comment",
        str(report_dir / "pr-comment.md"),
        "--output-source-segments",
        str(report_dir / "source-segments.jsonl"),
        "--output-target-segments",
        str(report_dir / "target-segments.jsonl"),
        "--output-mqm-judge-jsonl",
        str(report_dir / "mqm-judge.jsonl"),
        "--metric-cache",
        str(report_dir / "metric-cache.json"),
        "--qe-metric",
    ...
    if source_snapshot is not None:
        quality_cmd.extend(["--source", str(source_snapshot)])
    if env_flag("QUALITY_FAIL_ON_REJECT"):
        quality_cmd.append("--fail-on-reject")
    run(quality_cmd, cwd=repo_root)
